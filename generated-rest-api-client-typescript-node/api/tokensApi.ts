...
import { PaginatedTokenList } from '../model/paginatedTokenList';
...
export class TokensApi {
     ...
     * @param token 
     */
    public async tokensCreate (token?: Token, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Token;  }> {
        const localVarPath = this.basePath + '/api/tokens/';
    ...
    /**
     * Each user can have multiple authentication tokens. The reason to have multiple tokens per user is that he (or she) may consume REST API from multiple clients (or devices) using one single user account. User may then use a separate authentication token per each device or client.
     * @param digest A unique value identifying this auth token.
     */
    public async tokensDestroy (digest: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body?: any;  }> {
        const localVarPath = this.basePath + '/api/tokens/{digest}/'
        ...
        if (digest === null || digest === undefined) {
            throw new Error('Required parameter digest was null or undefined when calling tokensDestroy.');
        }
     ...
     */
    public async tokensList (filterSearch?: string, pageNumber?: number, pageSize?: number, sort?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaginatedTokenList;  }> {
        const localVarPath = this.basePath + '/api/tokens/';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/vnd.api+json'];
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PaginatedTokenList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "PaginatedTokenList");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
     ...
     * @param digest A unique value identifying this auth token.
     */
    public async tokensRetrieve (digest: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Token;  }> {
        const localVarPath = this.basePath + '/api/tokens/{digest}/'
            .replace('{' + 'digest' + '}', encodeURIComponent(String(digest)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        // verify required parameter 'digest' is not null or undefined
        if (digest === null || digest === undefined) {
            throw new Error('Required parameter digest was null or undefined when calling tokensRetrieve.');
        }
